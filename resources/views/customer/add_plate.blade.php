...
<div class="container py-5">
    ...
    <option value="" disabled {{ old('region') ? '' : 'selected' }}>Select Province</option>
    @foreach(['Sindh', 'Punjab', 'Balochistan', 'KPK'] as $region)
    ...
    @foreach(['Available', 'Pending', 'Sold'] as $region)
        <option value="{{ $region }}" {{ old('status') == $region ? 'selected' : '' }}>{{ $region }}</option>
    @endforeach
